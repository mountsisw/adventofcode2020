...
function processRecord(record: string, inputDisplay: HTMLDivElement, outputDisplay: HTMLDivElement)
{
    if (record.length > 0)
    {
        ...
        let first: Boolean = fields[2][Number(minMax[0]) - 1] == fields[1][0];
        let second: Boolean = fields[2][Number(minMax[1]) - 1] == fields[1][0];
        if ((first && !second) || (!first && second)) { answer++; outputDisplay.innerText = "Valid"; }
