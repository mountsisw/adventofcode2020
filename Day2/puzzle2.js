...
function init()
{
    ...
    appendToGrid(inputDiv, "Rule");
    appendToGrid(inputDiv, "Valid?");
    let validCount = 0;
    for (let loop1 = 0; loop1 < aValues.length; loop1++)
    {
        appendToGrid(inputDiv, aValues[loop1]);
        let parts = aValues[loop1].split(" ");
        let requiredChar = parts[1][0];
        let firstSecond = parts[0].split("-");
        let firstPos = Number(firstSecond[0]);
        let secondPos = Number(firstSecond[1]);
        let isValid = false;
        if (parts[2].length >= firstPos && parts[2][firstPos - 1] == requiredChar)
            isValid = (parts[2].length < secondPos) || (parts[2][secondPos - 1] != requiredChar);
        else isValid = (parts[2].length >= secondPos) && (parts[2][secondPos - 1] == requiredChar);
        if (isValid) { validCount++; appendToGrid(inputDiv, "Yes"); }
        else { appendToGrid(inputDiv, "No"); }
    }
    document.getElementById("output").innerText = validCount + " passwords are valid";    
}
