...
function init()
{
    ...
    for (let loop1 = 0; loop1 < aValues.length; loop1++)
    {
        ...
        let lastFound = -1;
        let numFound = 0;
        while ((lastFound = parts[2].indexOf(requiredChar, lastFound + 1)) >= 0) numFound++;
        let minmax = parts[0].split("-");
        if (numFound >= Number(minmax[0]) && numFound <= Number(minmax[1]))
        { validCount++; appendToGrid(inputDiv, "Yes"); }
