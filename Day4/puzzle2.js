...
function init()
{
    ...
    appendToGrid(inputDiv, "Passport");
    appendToGrid(inputDiv, "Valid?");
    let validCount = 0;
    let present = [false, false, false, false, false, false, false];
    let passportValues = "";
    ...
    for (let loop1 = 0; loop1 < aValues.length; loop1++)
    {
        if (aValues[loop1] == "")
        {
            appendToGrid(inputDiv, passportValues);
            if (present.includes(false)) { appendToGrid(inputDiv, "Invalid"); }
            else { appendToGrid(inputDiv, "Valid"); validCount++; }
            passportValues = "";
            present = [false, false, false, false, false, false, false];
        }
        else
        {
            let pairs = aValues[loop1].split(" ");
            for (let loop2 = 0; loop2 < pairs.length; loop2++)
            {
                let key = pairs[loop2].substr(0, 4);
                let value = pairs[loop2].substr(4);
                if (key == "byr:")
                {
                    if (value.length == 4)
                    {
                        let year = Number(value);
                        present[0] = (year >= 1920 && year <= 2002);
                    }
                }
                else if (key == "iyr:")
                {
                ...
                else if (key == "hgt:")
                {
                    ...
                    if (units == "cm") present[3] = (height >= 150 && height <= 193);
                    else if (units == "in") present[3] = (height >= 59 && height <= 76);
                }
                else if (key == "hcl:")
                {
                    present[4] = value.match(/#[0-9a-f]{6}/) ? true : false;
                }
                else if (key == "ecl:")
                {
                    present[5] = colors.includes(value);
                }
                else if (key == "pid:")
                {
                    if (value.length == 9) present[6] = value.match(/[0-9]{9}/) ? true : false;
                }
            }
            passportValues += aValues[loop1] + " ";
        }
    }
    if (passportValues != "")
    {
    ...
    document.getElementById("output").innerText = validCount + " valid passports";
}
