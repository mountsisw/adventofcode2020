...
function init()
{
    ...
    for (let loop1 = 0; loop1 < aValues.length; loop1++)
    {
        if (aValues[loop1] == "")
        {
            ...
            passportValues = "";
            if (present.length == keys.length) { appendToGrid(inputDiv, "Valid"); validCount++; }
            else appendToGrid(inputDiv, "Invalid");
            present = [];
        }
        else
        {
            ...
            {
                let key = pairs[loop2].substr(0, 3);
                if (keys.includes(key)) present.push(key);
            }
