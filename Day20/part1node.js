...
class Tile {
    ...
    addPresentations(top, left, right, bottom) {
        ...
        // this.doRotations(2, topFlip, right, left, bottomFlip, top, rightFlip, leftFlip, bottom);
        // this.doRotations(3, bottomFlip, rightFlip, leftFlip, topFlip, bottom, right, left, top);
    }
...
class PuzzleSolution {
    ...
    displayAnswer() {
        ...
        /* this.tiles.forEach((tile) =>
        {
            tile.presentations.forEach((presentation) =>
            {
                let matches: Array<TilePresentation>;

                if (this.tops.has(presentation.top))
                {
                    matches = this.tops.get(presentation.top);
                    matches.push(presentation);
                    this.tops.set(presentation.top, matches);
                }
                else
                {
                    matches = this.bottoms.get(presentation.top);
                    if (matches.length > 1) this.tops.set(presentation.top, [presentation]);
                }

                if (this.lefts.has(presentation.left))
                {
                    matches = this.lefts.get(presentation.left);
                    matches.push(presentation);
                    this.lefts.set(presentation.left, matches);
                }
                else
                {
                    matches = this.rights.get(presentation.left);
                    if (matches.length > 1) this.lefts.set(presentation.left, [presentation]);
                }

                if (this.tops.has(presentation.top) && this.lefts.has(presentation.left))
                {
                    matches = this.masks.has(presentation.mask) ? this.masks.get(presentation.mask) : new Array();
                    matches.push(presentation);
                    this.masks.set(presentation.mask, matches);
                }
            });
        }); */
        ...
        if (retVal) {
            console.log("Answer is " + (this.arrangement[0].tileID *
                ...
                this.arrangement[(this.maxTilesPerDirection * this.maxTilesPerDirection) - 1].tileID).toLocaleString());
        }
        else {
...
let fs = require('fs');
let dataFile = process.argv.length > 2 ? process.argv[2] : "Test1.txt";
let data;
try {
...
let puzzle = new PuzzleSolution();
let records = data.split("\n");
records.forEach((record) => {
    let len = record.length;
    if (record.substr(len - 1, 1) == "\r")
        record = record.substr(0, len - 1);
    puzzle.processRecord(record);
});
