...
class Tile
{
    ...
    public getArrangedData(presentationIndex = -1) : Array<string>
    {
        let presentation: TilePresentation = this.presentations[presentationIndex == -1 ? this.presentationUsed : presentationIndex];
        let copiedData: Array<string> = this.data.slice();
        if (presentation.flip == 1) copiedData.reverse();
        if (presentation.rotation == 0) return copiedData;
        let arrangedData: Array<string> = new Array();
        let arrangedLine: string;
...
class PuzzleSolution extends PuzzlePart
{
            ...
            else
            {
                if (this.tileRows == 0)
                {
                    for (let nLoop = 0; nLoop < record.length; nLoop++)
                        if (record.substr(nLoop, 1) == "#") this.topBorder += Math.pow(2, nLoop);
                }
                ...
                {
                    if (record.substr(0, 1) == "#") this.leftBorder += Math.pow(2, this.tileRows);
                    if (record.substr(maxBorderLength - 1, 1) == "#") this.rightBorder += Math.pow(2, this.tileRows);
                }
    ...
    private arrangements: Array<Array<TilePresentation>>;
    private maxTilesPerDirection: number;
    private tops: Map<number, Array<TilePresentation>>;
    // private rights: Map<number, Array<TilePresentation>>;
    // private bottoms: Map<number, Array<TilePresentation>>;
    private lefts: Map<number, Array<TilePresentation>>;
    private masks: Map<number, Array<TilePresentation>>;
    ...
    public displayAnswer()
    {
            ...
            this.tiles.forEach((tile) =>
        {
            tile.presentations.forEach((presentation) =>
            {
                ...
                let presentations: Array<TilePresentation>;
        ...
        if (answer == "") this.answerDisplay.innerHTML = "No solutions found";
        else this.answerDisplay.innerHTML = answer;
        ...
        let maxRoughness = 0;
        let maxMonsters = 0;
        let maxArrangement: number;
        answer = "";
        this.arrangements.forEach((arrangement, arrangementIndex) =>
        {
            let picture: Array<string> = new Array(this.maxTilesPerDirection * (maxBorderLength - 2));
            picture.fill("");
            arrangement.forEach((presentation, index) =>
            {
                let tileData: Array<string> = this.tiles.get(presentation.tileID).getArrangedData(presentation.index);
            ...
            {
                maxMonsters = monsterCount; maxArrangement = arrangementIndex;
                maxRoughness = 0;
    ...
    private hereBeMonsters(picture: Array<string>) : number
    {
        let monsterLine1 = /..................#./;
        let monsterLine2 = /#....##....##....###/g;
        let monsterLine3 = /.#..#..#..#..#..#.../;
        ...
        for (let lineLoop = 1; lineLoop < picture.length - 1; lineLoop++)
        {
            for (let match of picture[lineLoop].matchAll(monsterLine2))
            {
                ...
                if (monsterLine3.test(line3Target))
                {
                    ...
                    else console.log("No match on line " + (lineLoop - 1));
                }
                else console.log("No match on line " + (lineLoop + 1));
            }
        }
    ...
    public dumpData()
    {
        ...
        this.tiles.forEach(tile => 
        {
            answer += this.tableRow(String(tile.id), "<pre>" + tile.getArrangedData(0).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(1).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(2).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(3).join("\n") + "</pre>");
            answer += this.tableRow("&nbsp;", "&nbsp;", "&nbsp;", "&nbsp;", "&nbsp;");
            answer += this.tableRow(String(tile.id) + " flipped", "<pre>" + tile.getArrangedData(4).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(5).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(6).join("\n") + "</pre>",
                "<pre>" + tile.getArrangedData(7).join("\n") + "</pre>");
            answer += this.tableRow("&nbsp;", "&nbsp;", "&nbsp;", "&nbsp;", "&nbsp;");
            }, this);
        this.answerDisplay.innerHTML = answer;
    }

    private tableRow(...cellText: Array<string>)
    {
        let rowText = "<tr>"
