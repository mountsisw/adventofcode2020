        ...
        if (record.length > 0) {
            for (let nLoop = 0; nLoop < record.length; nLoop++) {
                if (record.substr(nLoop, 1) == "#")
                    this.activeCubes.push(new Cube(nLoop, this.recordCount, 0).toString());
            }
            this.recordCount++;
        }
        return true;
...
class Cube {
    constructor(x, y, z, active = true) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.active = active;
    }
    toString() { return String(this.x) + "|" + String(this.y) + "|" + String(this.z); }
    activeNext(matrix) {
