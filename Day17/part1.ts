...
class PuzzleSolution extends PuzzlePart
{
    ...
    public processRecord(record: string) : Boolean
    {
        if (record.length > 0)
        {
            ...
            {
                if (record.substr(nLoop, 1) == "#") this.activeCubes.push(new Cube(nLoop, this.recordCount, 0).toString());
            }
            this.recordCount++;
        }
        return true;
            ...
            this.activeCubes.forEach((cube) => 
            {
                ...
                for (let xLoop = x - 1; xLoop <= x + 1; xLoop++)
                    for (let yLoop = y - 1; yLoop <= y + 1; yLoop++)
                        ...
                        {
                            let newCube = new Cube(xLoop, yLoop, zLoop);
...
class Cube
{
    constructor(public x: number, public y: number, public z: number, public active: boolean = true) { }
    public toString() { return String(this.x) + "|" + String(this.y) + "|" + String(this.z); }
